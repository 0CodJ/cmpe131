...
interface TimelineSliderProps {
  ...
  onBoundsChange?: (bounds: { minYear: number; maxYear: number } | null) => void;
}
...
export function TimelineSlider({
  ...
  const calculateBounds = useCallback(() => {
    ...
    // Right bound is the latest event year
  ...
  // Check if we have actual events or are using defaults
  const relevantEvents = events.filter(
    (e) => (selectedMonth === 0 || e.month === selectedMonth) && (selectedDay === 0 || e.day === selectedDay)
  );
  const hasActualEvents = relevantEvents.length > 0;
  ...
  // Only set bounds when we have actual events - don't use default bounds for filtering
  useEffect(() => {
    if (onBoundsChange && hasActualEvents) {
      onBoundsChange({ minYear, maxYear });
    } else if (onBoundsChange && !hasActualEvents) {
      // When there are no events, clear the bounds so filtering doesn't happen
      onBoundsChange(null);
    }
  }, [minYear, maxYear, onBoundsChange, hasActualEvents]);
