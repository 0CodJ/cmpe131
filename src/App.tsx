...
import { Calendar, Clock, Tag, Search, Database, Globe, Plus, Minus, LogIn, Edit } from 'lucide-react';
...
function App() {
  ...
  const [selectedEventForSuggestion, setSelectedEventForSuggestion] = useState<CombinedEvent | null>(null);
  ...
  const searchEvents = useCallback(async () => {
    ...
    try {
      ...
      if (showApiEvents) {
        ...
        const formattedApiEvents: CombinedEvent[] = apiEvents.map((event: ApiHistoricalEvent, index: number) => ({
          id: `api-${event.year}-${selectedMonth}-${selectedDay}-${index}-${event.text.substring(0, 30).replace(/[^a-zA-Z0-9]/g, '')}`, // Create a truly unique ID
  ...
  const getSliderRange = () => {
    const min = Math.max(earliestYear, 1); // Use earliest year as minimum, but at least 1
    const max = 2025; // Target maximum
    
    // Adjust max to be reachable with current step size
    // Find the highest multiple of zoomLevel that's <= max
    const adjustedMax = Math.floor(max / zoomLevel) * zoomLevel;
    
    return {
      min: min,
      max: Math.max(adjustedMax, min + zoomLevel) // Ensure max is at least min + step
    };
  };
  ...
  const handleSuggestChange = (event: CombinedEvent) => {
    if (!user) {
      handleAuthClick('login');
      return;
    }
    setSelectedEventForSuggestion(event);
  ...
  return (
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        {/* Header section with title, subtitle, and authentication */}
          ...
          <div className="text-center">
            ...
            {user && (
              <p className="text-blue-400 text-sm mt-2">
                Welcome back! You can now suggest changes to historical events.
              </p>
            )}
          </div>
        </header>
        ...
        <div className="bg-slate-800/50 backdrop-blur-sm rounded-2xl shadow-2xl p-6 mb-8 border border-slate-700">
          ...
          <div className="space-y-4">
            ...
            <div className="relative w-full h-8 flex items-center group">
              ...
              {/* Custom slider thumb - horizontal blue bar */}
              <div 
                className="absolute w-4 h-8 bg-gradient-to-r from-blue-500 to-blue-400 rounded-lg shadow-lg border border-white transform -translate-y-1/2 top-1/2 cursor-pointer hover:scale-105 transition-all duration-200 z-10"
                style={{
                  left: `calc(${((selectedYear - sliderRange.min) / (sliderRange.max - sliderRange.min)) * 100}% - 8px)`
                }}
              ></div>
              ...
              <input
                ...
                className="absolute inset-0 w-full h-8 bg-transparent appearance-none cursor-pointer opacity-0 z-20"
              />
            </div>
              ...
              >
                ...
                <div className="mb-3">
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <h4 className="text-xl font-semibold text-white leading-relaxed flex-1 mr-4">{event.title}</h4>
                    {/* Suggest changes button */}
                    <button
                      onClick={() => handleSuggestChange(event)}
                      className="flex items-center gap-2 px-3 py-1 bg-yellow-500/20 hover:bg-yellow-500/30 text-yellow-300 rounded-lg transition text-sm font-medium"
                      title={user ? 'Suggest changes to this event' : 'Sign in to suggest changes'}
                    >
        ...
        <SuggestionModal
          ...
          onClose={() => {
            ...
            setSelectedEventForSuggestion(null);
          }}
          event={selectedEventForSuggestion}
